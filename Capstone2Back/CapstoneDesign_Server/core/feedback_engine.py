...
class FeedbackEngine:
    ...
    def _build_evaluation_prompt(self, data: Dict[str, Any], rubric: str, detailed: Dict[str, Any], persona: str = "soft") -> str:
        ...
        detailed_str = json.dumps(detailed, ensure_ascii=False, indent=2)
...
2. **비매너/태도 감지 (AI 특화)**: 단순 좌표 데이터에서 읽어낼 수 없는 '주머니에 손 넣기', '뒷짐 지기', '불안정한 다리 떨림', '지나친 머리 만지기' 등 비언어적 비매너 동작이 의심되는 구간을 논리적으로 추론하여 지적하십시오.
3. **Action Plan**: 개선을 위해 바로 실천할 수 있는 구체적인 팁을 3가지 이상 제시하십시오.
